...
createShape = function (description) {
    ...
    switch (description.type) {
        ...
        case 'capsule':
            cache_key =
                'capsule_' + description.radius + '_' + description.height;
            if ((shape = getShapeFromCache(cache_key)) === null) {
...
public_functions.addObject = function (description) {
    ...
    if (description.materialId !== undefined) {
        ...
        rbInfo.set_m_restitution(
            _materials[description.materialId].restitution
        );
    }
    ...
    if (_vehicles[description.id] !== undefined) {
        ...
        if (description.tuning !== undefined) {
            ...
            tuning.set_m_suspensionDamping(
                description.tuning.suspension_damping
            );
...
public_functions.removeObject = function (details) {
    ...
    if (_compound_shapes[details.id])
        Ammo.destroy(_compound_shapes[details.id]);
    ...
    if (world) {
        ...
        if (!params.timeStep) {
            if (last_simulation_time) {
                params.timeStep = 0;
                while (
                    params.timeStep + last_simulation_duration <=
                    fixedTimeStep
                ) {
                    params.timeStep =
                        (Date.now() - last_simulation_time) / 1000; // time since last simulation
                }
            } else {
        ...
        last_simulation_duration =
            (Date.now() - last_simulation_duration) / 1000;
        last_simulation_time = Date.now();
    }
};
...
reportConstraints = function () {
    ...
    if (i !== 0) {
        if (SUPPORT_TRANSFERABLE) {
            transferableMessage(constraintreport.buffer, [
                constraintreport.buffer,
            ]);
        } else {
